...
**平台可见性（重要）**：`notify_voice` **在桌面端可用**；但**CLI / TUI 会话里看不到它** ——
这两个入口走 `platform_toolsets.<platform>` 白名单，user 插件的自建 toolset 不被该列表承认
（写进去会被判 `unknown toolset`）。CLI/TUI 下请直接跑脚本：`python <脚本> "文本"`（等同 skill 形态的行为）。
